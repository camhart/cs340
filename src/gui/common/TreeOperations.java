...
public final class TreeOperations {
	 ...
	 */
	public static void selectTreeNode(JTree tree,
			Point point) {
		int itemRow = tree.getRowForLocation(
				(int) point.getX(),
				(int) point.getY());
		if (itemRow >= 0) {
			Rectangle rowBounds = tree
					.getRowBounds(itemRow);
	 ...
	 * 
	 * @param tree
	 *            tree object in which the selection is to be made
	 * @param node
	 *            node to be selected
	 * 
	 ...
	 */
	public static DefaultMutableTreeNode getSelectedTreeNode(
			JTree tree) {
